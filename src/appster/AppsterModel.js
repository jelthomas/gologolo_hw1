export default class AppsterModel {
    ...
    goList(newName){
        ...
        for(let i = 0; i < length; i++){
            if(newName == this.recentWork[i].name){
                ...
                this.view.showConfirmModalDialog();
                return;
            }
        }
        let myLogo = this.createNewWork(newName);
        this.appendWork(myLogo);
        this.view.hideTextInputDialog();
     ...
     * can be edited.
     */
    loadNewList(workName) {
        this.listToEdit = this.createNewWork(workName);
