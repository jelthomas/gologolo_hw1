...
export default class AppsterView {
    ...
    loadRoot(appData) {
        ...
        if (!appsterRootDiv) {
            ...
            let appsterIllegalLengthConfirmModal = this.buildAppsterIllegalLengthConfirmModal();
            let appsterTextInputModal = this.buildAppsterTextInputModal();
            let appsterChangeTextInputModal = this.buildAppsterChangeTextInputModal(); //Overridden
            ...
            appsterRootDiv.appendChild(appsterIllegalLengthConfirmModal);
            appsterRootDiv.appendChild(appsterTextInputModal);
            appsterRootDiv.appendChild(appsterChangeTextInputModal);
    ...
    buildAppsterIllegalLengthConfirmModal() {
        let confirmModal = this.buildElement( AppsterHTML.DIV, 
                                            AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL,
                                            [AppsterGUIClass.APPSTER_MODAL],
                                            [],
                                            null,
                                            AppsterGUIClass.MODAL_ANIMATION_LEFT);
        let confirmFrame = this.buildElement( AppsterHTML.DIV, 
                                            AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_FRAME,
                                            [AppsterGUIClass.APPSTER_MODAL_FRAME]);
        let header = this.buildElement( AppsterHTML.HEADER, 
                                        AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_HEADER,
                                        [AppsterGUIClass.APPSTER_MODAL_HEADER]);
        let section = this.buildElement(    AppsterHTML.SECTION, 
                                            AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_SECTION,
                                            [AppsterGUIClass.APPSTER_MODAL_SECTION]);
        let p = this.buildElement(AppsterHTML.P);
        let strong = this.buildElement(     AppsterHTML.STRONG, 
                                            "",
                                            [],
                                            [],
                                            AppsterText.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_PROMPT_TEXT);
        let okButton = this.buildElement(   AppsterHTML.BUTTON, 
                                            AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_OK_BUTTON,
                                            [AppsterGUIClass.APPSTER_MODAL_BUTTON],
                                            [],
                                            AppsterText.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_OK_BUTTON_TEXT);
        let footer = this.buildElement(     AppsterHTML.FOOTER, 
                                            "", 
                                            [AppsterGUIClass.APPSTER_MODAL_FOOTER],
                                            [],
                                            AppsterText.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL_FOOTER_TEXT);
        p.appendChild(strong);
        section.appendChild(p);
        confirmFrame.appendChild(header);
        confirmFrame.appendChild(section);
...
//        this.controller.setupCallback(newA, AppsterHTML.ONCLICK, AppsterCallback.APPSTER_PROCESS_EDIT_WORK, callbackArguments);
   ...
   /**
    * This method is for hiding the change name dialog.
    */
   hideTextInputDialog(){
   ...
   /**
    * This method is for showing the duplicate Logo name error dialog.
    */
   showConfirmModalDialog() {
    ...
    /**
    * This method is for hiding the duplicate Logo name error dialog.
    */
   hideConfirmModalDialog() {
        let dialog = document.getElementById(AppsterGUIId.APPSTER_CONFIRM_MODAL);
        dialog.classList.remove(AppsterGUIClass.IS_VISIBLE);
    }

    /**
    * This method is for showing the illegal length Logo name error dialog.
    */
   showIllegalLengthConfirmModalDialog() {
        let dialog = document.getElementById(AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL);
        dialog.classList.add(AppsterGUIClass.IS_VISIBLE);
    }

    /**
    * This method is for hiding the illegal length Logo name error dialog.
    */
   hideIllegalLengthConfirmModalDialog() {
        let dialog = document.getElementById(AppsterGUIId.APPSTER_ILLEGAL_LENGTH_CONFIRM_MODAL);
        dialog.classList.remove(AppsterGUIClass.IS_VISIBLE);
    }

   /**
    * This method is for showing the change name dialog.
    */
   showTextInputDialog(){
        let dialog = document.getElementById(AppsterGUIId.APPSTER_TEXT_INPUT_MODAL);
        let text = document.getElementById(AppsterGUIId.APPSTER_TEXT_INPUT_MODAL_TEXTFIELD);
        text.value = "";
        dialog.classList.add(AppsterGUIClass.IS_VISIBLE);
   }
